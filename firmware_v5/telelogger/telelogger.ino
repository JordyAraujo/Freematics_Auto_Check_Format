...
PID_POLLING_INFO obdData[] = {
    ...
    {PID_COOLANT_TEMP, "Coolant Temperature"},
    {PID_SHORT_TERM_FUEL_TRIM_1, "Short Term Fuel Trim 1"},
    {PID_LONG_TERM_FUEL_TRIM_1, "Long Term Fuel Trim 1"},
    {PID_SHORT_TERM_FUEL_TRIM_2, "Short Term Fuel Trim 2"},
    {PID_LONG_TERM_FUEL_TRIM_2, "Long Term Fuel Trim 1"},
    {PID_FUEL_PRESSURE, "Fuel Pressure"},
    {PID_INTAKE_MAP, "Intake MAP"},
    {PID_RPM, "RPM"},
    {PID_SPEED, "Speed"},
    {PID_TIMING_ADVANCE, "Timing Advance"},
    {PID_INTAKE_TEMP, "Intake Temp"},
    {PID_MAF_FLOW, "MAF Flow"},
    {PID_THROTTLE, "Throttle"},
    {PID_AUX_INPUT, "Auxiliar Input"},
    {PID_RUNTIME, "Runtime"},
    {PID_DISTANCE_WITH_MIL, "Distance With MIL On"},
    {PID_COMMANDED_EGR, "Commanded EGR"},
    {PID_EGR_ERROR, "EGR Error"},
    {PID_COMMANDED_EVAPORATIVE_PURGE, "Commanded Evaporative Purge"},
    {PID_FUEL_LEVEL, "Fuel Level"},
    {PID_WARMS_UPS, "Warms UPS"},
    {PID_DISTANCE, "Distance"},
    {PID_EVAP_SYS_VAPOR_PRESSURE, "Evaporator System Vapor Pressure"},
    {PID_BAROMETRIC, "Absolute Barometric Pressure "},
    {PID_CATALYST_TEMP_B1S1, "Catalyst Temperature: Bank 1, Sensor 1"},
    {PID_CATALYST_TEMP_B2S1, "Catalyst Temperature: Bank 2, Sensor 1"},
    {PID_CATALYST_TEMP_B1S2, "Catalyst Temperature: Bank 1, Sensor 2"},
    {PID_CATALYST_TEMP_B2S2, "Catalyst Temperature: Bank 2, Sensor 2"},
    {PID_CONTROL_MODULE_VOLTAGE, "Control Module Voltage"},
    {PID_ABSOLUTE_ENGINE_LOAD, "Absolute Engine Load"},
    {PID_AIR_FUEL_EQUIV_RATIO, "Air-Fuel Equivalent Ratio"},
    {PID_RELATIVE_THROTTLE_POS, "Relative Throttle Position"},
    {PID_AMBIENT_TEMP, "Ambient Temperature"},
    {PID_ABSOLUTE_THROTTLE_POS_B, "Absolute Throttle Position B"},
    {PID_ABSOLUTE_THROTTLE_POS_C, "Absolute Throttle Position C"},
    {PID_ACC_PEDAL_POS_D, "Acceleration Pedal Position D"},
    {PID_ACC_PEDAL_POS_E, "Acceleration Pedal Position E"},
    {PID_ACC_PEDAL_POS_F, "Acceleration Pedal Position F"},
    {PID_COMMANDED_THROTTLE_ACTUATOR, "Commanded Throttle Actuator"},
    {PID_TIME_WITH_MIL, "Time With MIL On"},
    {PID_TIME_SINCE_CODES_CLEARED, "Time Since Codes Cleared"},
    {PID_ETHANOL_FUEL, "Ethanol Fuel Percentage"},
    {PID_FUEL_RAIL_PRESSURE, "Fuel Rail Pressure"},
    {PID_HYBRID_BATTERY_PERCENTAGE, "Hybrid Battery Percentage"},
    {PID_ENGINE_OIL_TEMP, "Engine Oil Temperature"},
    {PID_FUEL_INJECTION_TIMING, "Fuel Injection Timing"},
    {PID_ENGINE_FUEL_RATE, "Engine Fuel Rate"},
    {PID_ENGINE_TORQUE_DEMANDED, "Engine Torque Demanded"},
    {PID_ENGINE_TORQUE_PERCENTAGE, "Engine Torque Percentage"},
    {PID_ENGINE_REF_TORQUE, "Engine Reference Torque"},
    {PID_BATTERY_VOLTAGE, "Battery Voltage"},
    {PID_DEVICE_TEMP, "Device Temperature"},
};

PID_POLLING_INFO obdSelect[] = {
    {PID_FUEL_LEVEL},
    {PID_SPEED},
    {PID_RPM},
    {PID_INTAKE_TEMP},
...
PID_POLLING_INFO obdList[53] = {};

PID_POLLING_INFO GPSLog[] = {
    // Defines which GPS PIDs would be LOGGED and their labels on the CSV
    {PID_GPS_DATE, "GPS Date"},
    {PID_GPS_TIME, "GPS Time"},
    {PID_GPS_LATITUDE, "GPS Lat"},
    {PID_GPS_LONGITUDE, "GPS Long"},
    {PID_GPS_ALTITUDE, "GPS Alt"},
    {PID_GPS_SPEED, "GPS Speed"},
    {PID_GPS_HEADING, "GPS Heading"},
    {PID_GPS_SAT_COUNT, "GPS Sat Count"},
    {PID_GPS_HDOP, "GPS HDOP"},
};
...
{
  if (!obdCheck)
  { // Compara os PIDs disponíveis com a lista de PIDs selecionados, criando uma nova lista apenas com os que serão utilizados
    int listCount = 0;
    for (byte i = 0; i < sizeof(obdData) / sizeof(obdData[0]); i++)
    {
      byte pid = obdData[i].pid;
      std::string name = obdData[i].name;
      if (!obd.isValidPID(pid))
        continue;
      int value;
      if (obd.readPID(pid, value))
      {
        for (byte j = 0; j < sizeof(obdSelect) / sizeof(obdSelect[0]); j++)
        {
          if (pid == obdSelect[j].pid)
          {
            obdList[listCount].pid = pid;
            obdList[listCount].name = name;
            // Serial.println(obdList[listCount].pid);
            Serial.println(obdList[listCount].name.c_str());
            listCount++;
          }
        }
      }
